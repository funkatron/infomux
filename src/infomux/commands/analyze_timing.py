...
def _detect_speech_segments(ffmpeg: Path, audio_path: Path) -> list[dict]:
    ...
    duration_match = re.search(
        r"Duration: (\d+):(\d+):(\d+\.\d+)", duration_result.stderr
    )
...
def execute(args: Namespace) -> int:
    ...
    for i, word in enumerate(sample_words):
        ...
        if _extract_frame_at_time(
            ...
            logger.warning(
                "failed to extract frame for word '%s' at %.2fs",
                word["text"],
                word["start_sec"],
            )
    ...
    if audio_path:
        ...
        logger.info(
            "detected %d speech segments from audio", len(actual_speech_segments)
        )
        for seg in actual_speech_segments[:5]:  # Show first 5
            logger.info(
                "  speech segment: %.2f-%.2fs (duration: %.2fs)",
                seg["start"],
                seg["end"],
                seg["end"] - seg["start"],
            )
        ...
        for i, word in enumerate(sample_words):
            ...
            if actual_speech_segments:
                for seg in actual_speech_segments:
                    ...
                    if not (
                        word["end_sec"] < seg["start"] or word["start_sec"] > seg["end"]
                    ):
