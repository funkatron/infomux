...
from infomux.audio_transcription import TRANSCRIPTION_AUDIO_FILENAME
...
class TestTranscribeStep:
    ...
    def test_builds_correct_command(self, tmp_path: Path) -> None:
        ...
        input_file = tmp_path / "audio.wav"
        input_file.touch()
        prepared_audio = tmp_path / TRANSCRIPTION_AUDIO_FILENAME
        prepared_audio.touch()
        ...
        with patch("infomux.steps.transcribe.get_tool_paths") as mock_tools:
            ...
            with patch(
                "infomux.steps.transcribe.ensure_transcription_audio",
                ...
                with patch("infomux.steps.transcribe.subprocess.run") as mock_run:
                    ...
                    assert str(prepared_audio) in cmd  # Prepared audio path
        ...
        with patch("infomux.steps.transcribe.get_tool_paths") as mock_tools:
            ...
            with patch(
                ...
                with patch("infomux.steps.transcribe.subprocess.run") as mock_run:
                    ...
                    assert outputs[1] == prepared_audio
